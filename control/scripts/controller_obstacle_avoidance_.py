...
class pure_pursuit :
    def __init__(self):
        ...
        rospy.Subscriber("/obstacle_detected", Bool, self.object_callback)
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_local_path == True and self.is_odom == True and self.is_status == True and self.is_scan_dist == True and self.is_scan_pose == True:
                ...
                rospy.loginfo(f"lc_complete: {self.is_lc_complete}")
                # ================================================================
    ...
    def lc_complete(self):
        ...
        if self.lane_flag == 1:
            target_lane_point = self.global_path_lane_2.poses[self.current_waypoint + 1].pose.position
        else:
            target_lane_point = self.global_path_lane_1.poses[self.current_waypoint + 1].pose.position

        distance_to_target_lane = sqrt((current_position[0] - target_lane_point.x)**2 + (current_position[1] - target_lane_point.y)**2)

        if distance_to_target_lane < 20.0 and self.status_msg.velocity.x * 3.6 > 0:
            self.is_lc_complete = True
            return True
        else:
            self.is_lc_complete = False
